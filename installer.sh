#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpVibes/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpVibes

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpVibes
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpVibes
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpVibes/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpVibes-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#           CiefpVibes INSTALLED SUCCESSFULLY           #"
